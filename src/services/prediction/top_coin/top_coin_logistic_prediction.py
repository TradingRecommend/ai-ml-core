...
class TopCoinLogisticPrediction(PredictionBase):
    def __init__(self, date):
        self.date = date
        self.logger = Logger(TopCoinLogisticPrediction.__name__)
    ...
    def get_model(self):
        ...
        model_uri = f'''models:/{ModelName.TOP_COIN_LOGISTIC_REGRESSION.value}/{ModelStage.get_state()}'''
    ...
    def run(self):
        ...
        for index, row in top_coin_features_df.iterrows():
            X = pd.DataFrame([row[TOP_COIN_LOGISTIC_FEATURES]], columns=TOP_COIN_LOGISTIC_FEATURES)
