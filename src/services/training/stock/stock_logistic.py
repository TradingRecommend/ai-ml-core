...
class StockLogisticModel(TrainMLModelBase):
    ...
    def run(self):
        ...
        X = stock_features_df[LOGISTIC_FEATURES].dropna()  # Loại bỏ các giá trị NaN
        y = stock_features_df['label'].loc[X.index]  # Nhãn tương ứng với các feature
        ...
        # correlations = stock_features_df[LOGISTIC_FEATURES].apply(lambda x: x.corr(stock_features_df['label']))
